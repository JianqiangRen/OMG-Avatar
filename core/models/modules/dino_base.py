...
class DINOBase(nn.Module):
    def __init__(self, output_dim=128, only_global=False):
        ...
        if not only_global:
            in_dim = self.dino_model.blocks[0].attn.qkv.in_features # 768
            hidden_dims=256
    ...
    def forward(self, images, output_size=None):
        ...
        if self.only_global:
            ...
            image_features = self.dino_model.get_intermediate_layers(images, 4) # four tensors of size  (bs,1369,768)
            out_features = []
            ...
            if output_size is not None:
                out = nn.functional.interpolate(out, output_size, mode="bilinear", align_corners=True) # (bs,256,296,296)
            out_global = image_features[-1][:, 0]  # (bs, 768)
            out_multiscale = image_features[-1]  # (bs, 1369, 768)
            return out, out_global, out_multiscale
