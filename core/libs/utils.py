...
class ConfigDict(dict):
    def __init__(self, model_config_path=None, data_config_path=None, init_dict=None):
        if init_dict is None:
            ...
            # timeInTokyo = timeInTokyo.astimezone(ZoneInfo('Asia/Beijin'))
            time_string = timeInTokyo.strftime("%b%d_%H%M_")
            rand_string =  "".join(random.sample('zyxwvutsrqponmlkjihgfedcba',5))
            config_dict['TRAIN']['EXP_STR'] = experiment_string

            func_id = config_dict['MODEL'].get('FUNC_ID', -1)
            metric_id = config_dict['MODEL'].get('METRIC_ID', -1)
            config_dict['TRAIN']['TIME_STR'] =  config_dict['TRAIN']['VERSION'] +  time_string +"_"+ "FUNCID{}_".format(func_id) +'_' + rand_string
        else:
